# Create a comprehension that returns a list of all the locations that have an exit to the forest. ( aka .key = 5
...
result = [(location, locations[location]) for location, ways in exits.items() if target in ways.values()]

print(result)

for each in locations:
    result = [(location, locations[location]) for location, ways in exits.items() if each in ways.values()]
    print(result)
